    ...
    sections: &[
        ...
        Section {
            ...
            blocks: &[
                ...
                Block::Para(
                    "**Writing programs in Xag costs you nothing.** Every program the \
                     compiler builds has the Xag runtime linked into it, so a piece of \
                     the project ends up inside what you ship. The exception is there \
                     for exactly that: those embedded portions may be redistributed \
                     without carrying any notice, so a program built with Xag owes \
                     nothing to anybody and need not mention the project anywhere.",
                ),
                Block::Para(
                    "**Changing Xag itself is where credit is due.** Distribute a \
                     modified compiler, runtime or tool and the Apache terms apply in \
                     full — keep the notices, and say plainly which files you changed.",
                ),
                Block::Para(
